...
<!-- Fetched from raw GitHub spec on 2026-04-29. -->
...
DESIGN.md may embed design tokens in a structured format. The system used to describe design tokens is inspired by the Design Token JSON spec. Specifically, it adopts typed token groups (colors, typography, spacing) and the `{path.to.token}` reference syntax for cross-referencing values.
...
Below is the schema for design tokens defined in front matter:
...
**Color**: A color value must start with `#` followed by a hex color code in SRGB color space.

**Typography** supports these properties:
...
- `lineHeight` (Dimension | number) - Accepts either a Dimension (e.g., `24px`, `1.5rem`) or a unitless number (e.g., `1.6`). A unitless number represents a multiplier of the element's `fontSize`, which is recommended CSS practice.
...
Every `DESIGN.md` follows the same structure. Sections can be omitted if not relevant to project, but those present should appear in sequence listed below. All sections use `<h2>` (`##`) headings. An optional `<h1>` heading may appear for document titling purposes but is not parsed as a section.
...
This section is a holistic description of a product's look and feel. It defines brand personality, target audience, and emotional response UI should evoke, such as playful or professional, dense or spacious. It serves as foundational context for guiding agent high-level stylistic decisions when a specific rule or token is not explicitly defined.

## Colors

This section defines color palettes for design system.

At least `primary` color palette must be defined, and additional color palettes may be defined as needed.

When there are multiple color palettes, design system may assign semantic role for each palette. Common convention names palettes in this order: `primary`, `secondary`, `tertiary`, and `neutral`.
...
- **Tertiary (#B8422E):** A vibrant earthy red as the sole driver for interaction, used exclusively for primary actions and critical highlights.
...
The `colors` section defines all color design tokens. The color tokens should be derived from key color palettes defined in markdown prose. Exact mapping from color palettes to color tokens may follow any consistent naming convention.

It is a `map<string, Color>` that maps color token name to value.
...
Most design systems have 9-15 typography levels. Design system may prescribe a role for each typography level.

Common naming convention for typography levels uses semantic categories such as `headline`, `display`, `body`, `label`, `caption`. Each category may further be divided into sizes, such as `small`, `medium`, and `large`.
...
- **Labels:** Space Grotesk is used for all technical data, timestamps, and metadata. Its geometric construction evokes the precision of a digital stopwatch. Labels are strictly uppercase with generous letter spacing.
```

### Design Tokens

The `typography` section defines precise font properties for typography design tokens.
...
This section describes layout and spacing strategy.
...
The layout follows a **Fluid Grid** model for mobile devices and a **Fixed-Max-Width Grid** for desktop (max 1200px).
...
The spacing section defines spacing design tokens. These may include spacing units useful for implementing layout model, such as column spans, gutters, and margins.

It is a `map<string, Dimension | number>` that maps spacing scale identifier to a dimension value or a unitless number (e.g., column counts or ratios).
...
This section describes how visual hierarchy is conveyed based on design style. If elevation is used, it defines required styling (spread, blur, color). For flat designs, this section explains alternative methods used to convey visual hierarchy (e.g., borders, color contrast).
...
Depth is achieved through **Tonal Layers** rather than heavy shadows. The background uses a soft off-white or very light green, while primary content sits on pure white cards.
```
...
The shape language is defined by **Architectural Sharpness**. All interactive elements, containers, and inputs utilize a minimal **4px corner radius**. This provides just enough softness to feel modern while maintaining a rigid, engineered aesthetic.
```

### Design Tokens

The `rounded` section defines design tokens for rounded corners used in buttons, cards, and other rectangular shapes.
...
This section provides style guidance for component atoms within design system. Common component types:
...
> Note: Components specification is actively evolving. Current structure provides intentional flexibility for domain-specific component definitions while spec matures.

### Design Tokens

The components section defines a collection of design tokens used to ensure consistent styling of common components. It is a `map<string, map<string, string>>` that maps component identifier to a group of sub-token names and values. Design token values may be literal values, or references to previously defined design tokens.

**Variants**. A component may have a variant for different UI states such as active, hover, pressed, etc. Those variant components may be defined under a different but related key, for example, `button-primary`, `button-primary-hover`, `button-primary-active`. The agent will consider all variants and make appropriate styling decisions.
...
The following names are commonly used across design systems. They are not required but provided as guidance for consistency.
